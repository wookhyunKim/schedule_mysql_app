...
class _TodoListState extends State<TodoList> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: todoList.isEmpty? Colors.white : Colors.blueGrey[100],
        title: Padding(
  ...
  addJSONData() async {     // 완료된 항목 보기가 true이면 다 포함된 url, false이면 제외한 url
    var url = Uri.parse(
      showDoneList
      ? 'http://localhost:8080/Flutter/semi_scheduler_Jin/select_done_o_flutter.jsp'        // 모든 데이터가 포함된 리스트 불러오기
      : 'http://localhost:8080/Flutter/semi_scheduler_Jin/select_done_x_flutter.jsp'        // 선택되지 않은(checkvalue=0)인 리스트만 불러오기
    );
  ...
  updateCheckValue(int seqNo, String todolist, int checkvalue) async {
    var url = Uri.parse(
      'http://localhost:8080/Flutter/semi_scheduler_Jin/update_checkvalue_flutter.jsp?seq=$seqNo&listname=$todolist&checkvalue=$checkvalue'
    );
    await http.get(url);
    addJSONData();
    setState(() {
  ...
  insertList() async {      // 테이블에 추가
    if(addListController.text.trim().isEmpty) {
      ...
      var url = Uri.parse(
        'http://localhost:8080/Flutter/semi_scheduler_Jin/insert_flutter.jsp?checkvalue=0&listname=${addListController.text}'
      );
      var response = await http.get(url);
      var dataConvertedJSON = json.decode(utf8.decode(response.bodyBytes));
  ...
  updateDeleteStatus(int seqNo) async {
    var url = Uri.parse(
      'http://localhost:8080/Flutter/semi_scheduler_Jin/update_deletestatus_flutter.jsp?seq=$seqNo&deletestatus=1&checkvalue=0'
    );
    var response = await http.get(url);
    var dataConvertedJSON = json.decode(utf8.decode(response.bodyBytes));
    var result = dataConvertedJSON["result"];
